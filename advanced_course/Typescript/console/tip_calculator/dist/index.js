...
async function main() {
    ...
    while (true) {
        ...
        check = Number(await question("Your Check needs to be a number. (e.g., 50.00)"));
    }
    //Get the tip Percentage
    let percentage = Number(await question("What percentage of tip will you give? (e.g., 15 for 15%)"));
    while (true) {
        if (isValidNumber(percentage)) {
            break;
        }
        percentage = Number(await question("Your tip percentage needs to be a number. (e.g., 15 for 15%)"));
    }
    //Split the Check among multiple people?
    let isSplit = String(await question("Should the bill be split among multiple people? (yes/no)")).toLowerCase();
    while (true) {
        if (isSplit === "yes" || isSplit === "no") {
            break;
        }
        isSplit = String(await question(`Should the bill be split among multiple people?\nPlease write  "yes" or "no"`)).toLowerCase();
    }
    //Split between how many people?
    let splitAmount = 1;
    if (isSplit === "yes") {
        splitAmount = Number(await question("How many people will split the bill?"));
        while (true) {
